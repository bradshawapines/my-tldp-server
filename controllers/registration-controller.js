...
const registrationController = {
  ...
  getRegistrationByID: function(req, res) {
    Event.findOne({'REGISTRATION_ID': req.params.registration_id}, '', function(err, registration){
      if(err || ! registration) {
        res.sendStatus(404)
      } else {
        res.status(200).send(dataCleaner.cleanEvent(registration))
      }
    })
  },
  ...
  postRegistration: function(req, res) {
    Registration.create({ EVENT_ID: req.body.event_id, CUSTOMER_ID: req.body.customer_id, 
      REGISTRATION_DATE: req.body.registration_date, NOTES: req.body.notes}).then(
      (r) => { 
